 ...
 *
 */
public class SliceTheFiles {
    public static void main(String[] args) {
        try {
            File file = new File("C:/Users/Public/Videos/Sample Videos/Wildlife.wmv");//File read from Source folder to Split.
            if (file.exists()) {

            String videoFileName = file.getName().substring(0, file.getName().lastIndexOf(".")); // Name of the videoFile without extension
            File splitFile = new File("C:/Videos_Split/"+ videoFileName);//Destination folder to save.
            if (!splitFile.exists()) {
                splitFile.mkdirs();
